...
func (h *configEntity) parseOptionList(options []string) error {
	...
	for _, option := range options {
		...
		t, v := strings.TrimSpace(columns[0]), strings.TrimSpace(columns[1])
		...
		case "bits":
			...
			h.bits_per_line = n
			if n <= 0 {
				return fmt.Errorf("FATAL: Invalid value for 'bits' option (%s)", v)
			}
		case "numbers":
			...
			default:
				return fmt.Errorf("FATAL: Invalid value for 'numbers' option (%s)", v)
			}
